...
type Phase3Feature = 'rms_single' | 'mean_abs' | 'p2p' | 'peak_abs' | 'lockin_amp';
...
class App {
    ...
    private lockinPhaseDegInput: HTMLInputElement;
    ...
    private lockinPhaseGroup: HTMLElement;
    ...
    private bindSignalSource(document: Document): void {
        ...
        this.frequencySlider.addEventListener('input', onChange);
    }
    ...
    private bindDriverPanel(document: Document): void {
        ...
        if (this.liveDriverActionEl) {
            this.liveDriverActionEl.innerText = 'Off';
        }
    ...
    private getDriverSamples(): number {
        const parsed = this.parseNumber(this.driverSamplesInput.value, 256);
        return Math.max(1, Math.min(16384, Math.round(parsed)));
    }
    ...
    private executeDriverAction(): void {
        ...
        if (action === 'off') {
            this.writeDriverOutput('Off: no driver function active.');
            return;
        }
        ...
        if (action === 'config_text') {
            this.connector.getConfigAsText((text) => {
                this.writeDriverOutput(text);
                finalize();
            });
            return;
        }

        if (action === 'driver_state') {
            this.connector.getOutputChannel((channel) => {
                this.connector.getDacAmplitude((amplitude) => {
                    this.connector.getWaveformLength((len) => {
                        this.connector.getPlotDecimationMode((mode) => {
                            this.connector.getPlotDecimationMaxPoints((maxPoints) => {
                                this.connector.getAdcDecimationStep((adcStep) => {
                                    this.connector.getDacDecimationStep((dacStep) => {
                                        this.writeDriverOutput(
                                            'output_channel=' + channel.toString() +
                                            ', amplitude_vpk=' + amplitude.toFixed(4) +
                                            ', len=' + len.toString() +
                                            ', decimation_mode=' + this.getDriverDecimationModeLabel(mode) +
                                            ', n_plot_max=' + maxPoints.toString() +
                                            ', step_adc=' + adcStep.toString() +
        ...
        if (action === 'driver_sizes') {
            this.connector.getAdcSize((adcSize) => {
                this.connector.getDacSize((dacSize) => {
                    this.writeDriverOutput('adc_size=' + adcSize.toString() + ', dac_size=' + dacSize.toString());
                    finalize();
                });
            });
            return;
        ...
        if (action === 'adc_raw_mean') {
            this.connector.getAdcRawData(n, (adc0, adc1) => {
                this.writeDriverOutput('ADC raw mean (' + n.toString() + '): IN1=' + adc0.toString() + ', IN2=' + adc1.toString() + ' counts');
                finalize();
            });
            return;
        }

        if (action === 'adc_raw_rms') {
            this.connector.getAdcRmsData(n, (rms0, rms1) => {
                this.writeDriverOutput(
                    'ADC raw RMS (' + n.toString() + '): IN1=' + rms0.toFixed(4) +
                    ', IN2=' + rms1.toFixed(4) +
                    ' counts'
                );
                finalize();
        ...
        if (action === 'adc_snapshot') {
            this.connector.getAdcSnapshot((array) => {
                const preview = Array.prototype.slice.call(array, 0, previewCount).join(', ');
                this.writeDriverOutput(
                    'ADC snapshot len=' + array.length.toString() +
                    ', preview[0..' + Math.max(0, Math.min(previewCount, array.length) - 1).toString() + ']=[' + preview + ']'
                );
                finalize();
            });
            return;
        ...
        if (action === 'dac_snapshot') {
            this.connector.getDacSnapshot((array) => {
                const preview = Array.prototype.slice.call(array, 0, previewCount).join(', ');
                this.writeDriverOutput(
                    'DAC snapshot len=' + array.length.toString() +
                    ', preview[0..' + Math.max(0, Math.min(previewCount, array.length) - 1).toString() + ']=[' + preview + ']'
                );
                finalize();
            });
    ...
    private bindWorkflowControls(document: Document): void {
        ...
        this.lockinPhaseDegInput = <HTMLInputElement>document.getElementById('lockin-phase-deg');
        ...
        this.lockinPhaseGroup = <HTMLElement>document.getElementById('lockin-phase-group');
        ...
        this.lockinPhaseDegInput.addEventListener('change', onWorkflowChanged);
    ...
    private applySignalSourceSettings(): void {
        ...
        const frequencyCmd = this.parseNumber(this.frequencySlider.value, 10000);
        const outputChannel = Number(this.outputChannelSelect.value);
        const amplitudeCmd = this.parseNumber(this.amplitudeInput.value, 10.0);
        const frequencyEff = this.clampFrequencyHz(frequencyCmd);
        const amplitudeEff = this.clampAmplitudeVpk(amplitudeCmd);

        this.frequencySlider.value = Math.round(frequencyEff).toString();
        this.amplitudeInput.value = amplitudeEff.toFixed(3);
        ...
        this.frequencyValueEl.innerText = Math.round(frequencyEff).toString();
    ...
    private updateFormulaAndParameterVisibility(): void {
        ...
        const lockinPhaseDeg = this.getLockinPhaseDeg();
        const frequencyEff = this.clampFrequencyHz(this.parseNumber(this.frequencySlider.value, 10000));
        ...
        if (featureMethod === 'rms_single') {
            featureFormula = 'RMS = sqrt((1/N) * sum(x^2)), x from Phase-2 input';
        } else if (featureMethod === 'mean_abs') {
            featureFormula = 'Feature = (1/N) * sum(|x|)';
        } else if (featureMethod === 'p2p') {
            featureFormula = 'Feature = max(x) - min(x)';
        } else if (featureMethod === 'peak_abs') {
            featureFormula = 'Feature = max(|x|)';
        } else {
            featureFormula = 'I=(2/N)*sum(x*cos(w*i+phi0)), Q=(2/N)*sum(x*sin(w*i+phi0)), Feature=(1/sqrt(2))*sqrt(I^2+Q^2)';
        }
        ...
        this.lockinPhaseGroup.style.display = 'none';
        ...
        if (featureMethod === 'lockin_amp') {
            this.lockinPhaseGroup.style.display = 'block';
            this.liveLockinParamsEl.innerText = 'f_ref=' + frequencyEff.toFixed(1) + ' Hz, phi0=' + lockinPhaseDeg.toFixed(1) + ' deg';
        } else {
    ...
    private computeFeature(in1: number[], in2: number[]): number {
        ...
        if (method === 'rms_single') {
            return this.computeRms(x);
        }
        if (method === 'mean_abs') {
            return this.computeMeanAbs(x);
        }
        if (method === 'p2p') {
            return this.computePeakToPeak(x);
        }
        if (method === 'peak_abs') {
            return this.computePeakAbs(x);
        }

        return this.computeLockinAmplitude(x);
    }
    ...
    private computeLockinAmplitude(values: number[]): number {
        if (values.length === 0) {
            return 0;
        }

        const frequency = this.clampFrequencyHz(this.parseNumber(this.frequencySlider.value, 10000));
        const w = 2.0 * Math.PI * frequency / this.sampleRateHz;
        const phaseOffset = this.getLockinPhaseRad();
        let iAcc = 0;
        let qAcc = 0;

        for (let i = 0; i < values.length; i++) {
            const phase = w * i + phaseOffset;
            iAcc += values[i] * Math.cos(phase);
            qAcc += values[i] * Math.sin(phase);
        }

        const iComp = (2.0 / values.length) * iAcc;
        const qComp = (2.0 / values.length) * qAcc;
        const ampPeak = Math.sqrt(iComp * iComp + qComp * qComp);
        return ampPeak / Math.sqrt(2.0);
    }
    ...
    private getWindowSamples(): number {
        const parsed = this.parseNumber(this.rmsWindowSamplesInput.value, 4096);
        return Math.max(16, Math.min(16384, Math.round(parsed)));
    }
    ...
    private getLockinPhaseDeg(): number {
        const parsed = this.parseNumber(this.lockinPhaseDegInput.value, 0.0);
        return Math.max(-360.0, Math.min(360.0, parsed));
    }

    private getLockinPhaseRad(): number {
        return this.getLockinPhaseDeg() * Math.PI / 180.0;
    }
    ...
    private computeMeanAbs(values: number[]): number {
        if (values.length === 0) {
            return 0;
        }
    ...
    private computePeakToPeak(values: number[]): number {
        if (values.length === 0) {
            return 0;
        }
    ...
    private computePeakAbs(values: number[]): number {
        if (values.length === 0) {
            return 0;
        }
    ...
    private getDriverDecimationModeLabel(mode: number): string {
        if (mode === 0) return 'Off (1:1)';
        if (mode === 1) return 'Stride';
        if (mode === 2) return 'Min/Max envelope';
        return 'Mean Bucket';
    }
